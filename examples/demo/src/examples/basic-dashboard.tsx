...
import { Home, Users, BarChart3, Settings, Bell, TrendingUp, Activity, TrendingDown } from 'lucide-react'
...
const contentByNav: Record<NavItem, ContentData> = {
  home: {
    ...
    activityItems: [
      { text: 'New user registered', time: '2 minutes ago', color: 'bg-green-500' },
      { text: 'Payment received', time: '5 minutes ago', color: 'bg-blue-500' },
      { text: 'Server warning', time: '10 minutes ago', color: 'bg-yellow-500' },
      { text: 'Order completed', time: '15 minutes ago', color: 'bg-green-500' }
    ]
  },
  users: {
    ...
    activityItems: [
      { text: 'Sarah Williams signed up', time: '1 minute ago', color: 'bg-green-500' },
      { text: 'Mike Chen updated profile', time: '12 minutes ago', color: 'bg-blue-500' },
      { text: 'Emma Davis verified email', time: '45 minutes ago', color: 'bg-green-500' },
      { text: 'John Smith logged in', time: '1 hour ago', color: 'bg-blue-500' }
    ]
  },
  analytics: {
    ...
    activityItems: [
      { text: 'Traffic spike detected', time: '5 minutes ago', color: 'bg-green-500' },
      { text: 'New referral source', time: '20 minutes ago', color: 'bg-blue-500' },
      { text: 'Goal completed: 50 signups', time: '1 hour ago', color: 'bg-green-500' },
  ...
  settings: {
    ...
    activityItems: [
      { text: 'Settings backup created', time: '30 minutes ago', color: 'bg-green-500' },
      { text: 'API key rotated', time: '2 hours ago', color: 'bg-blue-500' },
      { text: 'Security scan completed', time: '4 hours ago', color: 'bg-green-500' },
        ...
        <BlockContent className="p-6">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-slate-200">
            <div className="p-6 border-b border-slate-200">
              <h3 className="text-lg font-semibold">{currentContent.tableTitle}</h3>
            </div>
                ...
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    {currentContent.tableHeaders.map((header, index) => (
                      <th key={index} className="px-6 py-3 text-left text-xs font-medium text-slate-600 uppercase">
                        {header}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody className="divide-y divide-slate-200">
                  {currentContent.tableData.map((row, index) => (
                    <tr key={index} className="hover:bg-slate-50">
                      <td className="px-6 py-4 text-sm text-slate-900">{row.col1}</td>
                      <td className="px-6 py-4 text-sm text-slate-600">{row.col2}</td>
                      <td className="px-6 py-4 text-sm text-slate-600">{row.col3}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
